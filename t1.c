...
void perform_Matrix_Multiplication(double *A[row1], double *B[row2])
{
    double *values = NULL;
    ...
    while (row < row1)
    {
        ...
        while (col < col2)
        {
            ...
            if (counter < n_Thread)
            {
                pthread_create(&threads[counter], NULL, thred_Matrix, (void *)values);
                counter++;
            }
            ...
            if (check)
            {
                ...
                while (i < counter)
                {
                    ...
                    fprintf(final_File, "%lf  ", *(double *)ptr_values);
...
void matrix_Reading_From_File()
{
    double matrix;
    fp = fopen("SampleMatricesWithErrors.txt", "r");
    final_File = fopen("z_matrixresults11111.txt", "w");
    ...
    while (fscanf(fp, "%d,%d", &row1, &col1) != EOF)
    {
        ...
        perform_Matrix_Multiplication(A, B);
    }
}
