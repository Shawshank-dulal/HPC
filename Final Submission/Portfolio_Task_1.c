...
#include <pthread.h>
#include <unistd.h>
/* This program multiplies matrices provided in SampleMatricesWithErrors.txt and saves them in file matrixresults2049822.txt.
    The multiplication of matrices happens in multiple threads and thread number can be input by the user
    To compile the program type: gcc Portfolio_Task_1.c -o <output_file_name> -pthread
    then to run the program type: ./<previously_given_file_name> <no_of_threads>
    Then check the output files for the resultant matrices */
...
void readMatrices()
{
    ...
    double matrix;
    fp = fopen("SampleMatricesWithErrors2.txt", "r");
